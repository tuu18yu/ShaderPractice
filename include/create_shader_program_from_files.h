...
inline bool create_shader_program_from_files(
  ...
  GLuint & id)
{
  const auto create_compile_attach = []( const std::vector<std::string> & paths, const GLenum type, const GLuint prog_id, GLuint & s) -> bool
  {
    // check which shader type is
    ...
    {
      // read each path and add to strs
      for(int p = 0; p < paths.size(); p++)
      {
        const auto & path = paths[p];
        std::ifstream t(path); // read file from path
        if(t.fail())
        {
        ...
        std::stringstream ss; // use stringstream to read 
        ss << t.rdbuf();
        if(p!=0)
        {
        ...
        total_length += ss.str().length();
      }
    }

    // convert each string to cstr
